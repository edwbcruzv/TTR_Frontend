...
const FormCaseStep = ({
  ...
  const [multimediaList, setMultimediaList] = useState(
    watch(`${name}_multimedia_list`) ? watch(`${name}_multimedia_list`) : []
  );
          ...
          <Grid item xs={8}>
            ...
            {!stepComplete && (
              <Typography variant="body1" color="initial">
                Se requiere entre {minCaracteres} a {maxCaracteres} caracteres
                para pasar a la siguiente seccion del caso
              </Typography>
            )}
            {stepComplete && (
              <Typography variant="body1" color="initial">
                Puede continuar al siguiente seccion.
              </Typography>
            )}
          </Grid>
          <Grid item xs={4}>
            <FilesUpload
              name={`${name}_multimedia_list`}
              setValue={setValue}
              multimedia={multimediaList}
              setMultimedia={setMultimediaList}
            />
          </Grid>
        </>
      )}
        ...
        <>
          ...
          <Grid item xs={12}>
            <TextField
              ...
              helperText={errors.introduccion && errors.introduccion.message}
            />
          </Grid>
        </>
      )}
        ...
        <>
          <Grid item xs={8}>
            <TextField
              ...
              variant="outlined"
              error={errors.anexos}
              helperText={errors.anexos && errors.anexos.message}
            />
          </Grid>
          <Grid item xs={4}>
            {/* <FilesUpload/> */}
